...
-- 3. Devuelve el nombre del producto, su precio y el nombre de su fabricante, del producto más barato.
	 /* SELECT 
        ...
        fabricante.nombre
	FROM producto
    INNER JOIN fabricante --SI USO LEFT JOIN DEVUELVE OTRO RESULTADO
...
-- 6. Devuelve un listado con todos los productos de los fabricantes Asus, Hewlett-Packard. Utilizando el operador IN.
	/* SELECT 
		producto.nombre as nombre_prod,
        fabricante.nombre as fabricante_nom
	FROM producto
    INNER JOIN fabricante
    ON fabricante.nombre IN ('Asus', 'Hewlett-Packard') */
...
-- 1. Devuelve los nombres de los fabricantes que tienen productos asociados. (Utilizando IN o NOT IN).
		/* SELECT
			nombre
		FROM fabricante
        WHERE codigo IN (SELECT codigo FROM producto) */
...
-- 1. Devuelve un listado con todos los nombres de los fabricantes que tienen el mismo número de productos que el fabricante Lenovo.
	/* SELECT
		nombre
	FROM fabricante
    WHERE (SELECT COUNT(*) FROM producto WHERE codigo_fabricante = 2) = (SELECT COUNT(*) FROM producto WHERE codigo_fabricante = fabricante.codigo) */
